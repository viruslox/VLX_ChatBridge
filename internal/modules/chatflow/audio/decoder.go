...
func DecodeMediaToPCM(id string, filePath string, routeSRT bool, routeDiscord bool, volume int) error {
...
func DecodeMP3ToPCM(filePath string) error {
	return DecodeMediaToPCM("chatflow_decoder", filePath, true, false, 100)
}

// PlayAlert is a helper to decode a specific file with routing flags.
func PlayAlert(id string, filePath string, routeSRT bool, routeDiscord bool, volume int) {
	if filePath == "" {
		return
	}
	err := DecodeMediaToPCM(id, filePath, routeSRT, routeDiscord, volume)
	if err != nil {
